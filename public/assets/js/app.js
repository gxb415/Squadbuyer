...
$('#user-submit').on('click', function(event){
    ...
    // using promise to get UserId and CampaignId, then post both to CampaignsUsers
    let UserId, CampaignId;
    
    database.query( 'SELECT UserId FROM Users' )
    .then( rows => {
        UserId = rows;
        return database.query( 'SELECT CampaignId FROM Campaigns' );
    } )
    .then( rows => {
        otherRows = rows;
    ...
    .then( () => {
        $.post('/api/newCampaignUserPair', UserId, CampaignId);
    } );
